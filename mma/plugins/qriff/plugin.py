...
def setTuning(trk):
    ...
    if not tuningSet:
        ...
        if not fTrack.upper() in mma.MMA.gbl.tnames:
            mma.MMA.alloc.trackAlloc('%s' % fTrack, 0)
            pu.addCommand("%s Copy %s" % (fTrack, trk))

        if not sTrack.upper() in mma.MMA.gbl.tnames:
            mma.MMA.alloc.trackAlloc('%s' % sTrack, 0)
